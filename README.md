...
- 🔒 **Optional password protection** (`--protect`): an elegant animated
  ...
  exemptions for health checks or webhooks and a password that survives
  rebuilds. This is real protection, not a flimsy `.htaccess` prompt: passwords
  are stored only as salted hashes, and repeated wrong attempts lock the client
  out automatically. The protection belongs to the environment itself, so an
  update run from the administration cannot silently remove it, and the
  Shopware updater sits behind the same login although it ships without one.
  Forgot the password? `--reset-password` issues a new one in seconds.
- 🕵️ **GDPR anonymization** (`--anonymize`): names, addresses, e-mail
  addresses, phone numbers, dates of birth and VAT ids become realistic fake
  values, customer passwords are dropped, and generated documents (invoices
  etc.) are left out. Newsletter recipients, reviews, logs, saved carts and
  personal access keys stay empty. Administration accounts receive one freshly
  generated password that is shown once at the end, and stored interface
  secrets are invalidated, so no credential from the copy still works on the
  live shop. A final check confirms the result and stops the run if anything
  real is left. Perfect for third parties: you can safely let external agencies
  work on this staging.
...
| `--protect-allow=<path>` | Paths reachable without password (e.g. `/health`), repeatable. `/api` and `/admin` are refused unless the administration accounts are replaced in the same run |
| `--reset-password <SRC>` | New password for an existing staging, no rebuild |
| `--anonymize` | GDPR: anonymize customer data, replace administration accounts, drop generated documents |
| `--reset-admin-passwords` | Replace the administration accounts without touching customer data |
| `--keep-admin-passwords` | Keep the administration accounts as they are, even with `--anonymize` |
